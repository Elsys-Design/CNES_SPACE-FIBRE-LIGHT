...
class SpaceFibre_Random_Generator:
    ...
    async def write_random_inputs(self, file_path, frame_number, frame_size, delay = 0, invert_polarity = 0, seed = 42):
        """
        ...
        number_of_words = frame_number * ceil((frame_size/4))
        ...
        #send idle word 100 times for simu
        for i in range(10):
            data_to_log = ""
            k_encoded_to_log = ""
        ...
        #send the number of frame needed
        for i in range(frame_number):
            ...
            #send id of the frame
            frame_number_binary = f"{(1+i):0>32b}"
            data_to_log = ""
            k_encoded_to_log = ""
            for n in range(4):
                    data_10b, k_encoded  = await self.write_to_Rx(frame_number_binary[32-8*(n+1):32-8*n], 0, 0, invert_polarity = invert_polarity)
                    data_to_log = data_10b + "_" + data_to_log
                    k_encoded_to_log = str(k_encoded) + k_encoded_to_log
            ...
            log_file.write("32;" + f"{(1+i):0>8X}" + ";0;0000;\n")
            #send the content of the pseudo random frame
            for j in range(ceil(frame_size/4)):
